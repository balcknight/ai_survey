...
| `merge_low_depth_abs` | 7 | 低深度区（depth<30）峰合并时的绝对深度差阈值，差值<=此值则合并 |
...
| `spe_left_min_threshold` | 0.75 | SpeFreq 主峰左侧最低点比例阈值（75%），超过此值判为异常 |
| `num_left_min_threshold` | 0.6 | NumFreq 主峰左侧最低点比例阈值（60%），超过此值判为异常 |
| `shoulder_min_freq_ratio` | 0.25 | 肩峰频率最低比例阈值（25%），肩峰频率需达到全局最高峰的此比例才保留，防止低深度噪声区误检 |
| `all_peaks_too_low_ratio` | 0.1 | 所有峰太低判定阈值（10%），检测到的最高峰频率低于depth>=10范围内全局最高值的此比例时判为异常（排除depth<10的错误k-mer噪声峰） |
