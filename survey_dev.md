...
1. 峰合并逻辑：
   ...
   - 遍历相邻峰，如果深度差 <= 15% * 当前峰深度再向上取整（`np.ceil(0.15*depth)`）则合并为一个峰
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `filepath` | 必填 | 输入文件路径（SpeFreq.cut 或 NumFreq.cut） |
...
| `prominence_ratio` | 0.01 | 峰高度相对最高峰的最小比例阈值（1%），低于此比例的峰视为噪声过滤掉 |
...
| `min_width` | 15 | 峰的最小半高全宽（FWHM，索引单位），宽度不足的窄峰被过滤 |
| `merge_tolerance` | 0.15 | 双文件峰合并时的深度容差（15%），深度差在此范围内的峰合并为同一峰 |
| `low_depth_threshold` | 10 | 低深度峰过滤阈值，深度小于此值的第一个峰在不成比例且高度不足时会被忽略 |
| `low_peak_freq_ratio` | 0.6 | 低深度峰高度比例阈值（60%），低深度峰高度不及最高峰此比例时忽略 |
...
abnormal_flags.append(left_ratio > 0.8)  # 超过80%标记异常
判定逻辑：

对每个检测到的峰，取其左侧（depth更小方向）平滑曲线的全局最小值 left_min
计算 left_min / peak_freq，即左侧最低点占峰高的比例
如果比例 > 0.8（80%），标记为异常峰型
含义：正常的生物学峰应该是左侧有明显下降再上升到峰值的形态。如果左侧鞍部几乎和峰一样高（>80%），说明这个"峰"其实不是一个真正的凸起，可能是噪声或数据质量问题导致的假峰。

注意：这个异常标记目前只用于打印输出（L158-159），并没有参与后续的 classify_peaks 判定逻辑——即异常峰仍然会被计入峰数和比值计算。
